		...
		<div class="modal-content">
			<form action="" method="POST" enctype="multipart/form-data">
				...
				<div class="modal-footer justify-content-between">
					<a href="kelas_template.php" class="btn btn-success btn-sm btn-flat" target="_blank"><i class="fas fa-download"></i> Download</a>
					<button type="submit" name="upload" class="btn btn-primary btn-sm btn-flat"><i class="fas fa-upload"></i> Upload</button>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<form action="" method="post">
				<div class="modal-body">
					<div class="col-sm-12">
						<input type="hidden" class="form-control form-control-sm col-sm-6" id="idthpel" name="idthpel" value="<?php echo $idtahun;?>">
						<input type="hidden" class="form-control form-control-sm col-sm-6" id="idrombel" name="idrombel">
						...
						<div class="form-group row mb-2">					
							...
							<input type="text" class="form-control form-control-sm col-sm-6" id="nmrombel" name="nmrombel" placeholder="Isikan Nama Rombel">
						</div>
						<div class="form-group row mb-2">					
							...
							<select class="form-control form-control-sm col-sm-6" id="idwalas" name="idwalas">
								<option value="">..Pilih..</option>
								<?php
									$qwls=$conn->query("SELECT idgtk, nama FROM tbgtk g INNER JOIN tbskul sk USING(idskul) LEFT JOIN tbrombel r USING(idgtk) GROUP BY g.idgtk");
	...
	<div class="card card-secondary card-outline">
		<div class="card-header">
			...
			<div class="card-tools">
				<button	button class="btn btn-flat btn-success btn-sm" id="btnTambah" data-toggle="modal" data-target="#myAddKelas">
					<i class="fas fa-plus-circle"></i>&nbsp;Tambah
				</button>
				<button class="btn btn-flat btn-info btn-sm" id="btnImport" data-toggle="modal" data-target="#myImportRombel">
			...
			<div class="form-group row mb-2">
				<div class="table-responsive">
					<table id="tb_kelas" class="table table-bordered table-striped table-sm">
						...
						<tbody>
							<?php
								$qk=$conn->query("SELECT rb.*,k.nmkelas, g.nama FROM tbrombel rb INNER JOIN tbkelas k USING(idkelas) INNER JOIN tbgtk g USING(idgtk) WHERE rb.idthpel='$idtahun'");
								$no=0;
							...
							<tr>
								...
								<td style="text-align: center">
									<a href="#myAddKelas" data-toggle="modal" data-id="<?php echo $m['idrombel'];?>" class="btn btn-xs btn-success btn-flat btnUpdate">
										<i class="fas fa-edit"></i>&nbsp;Edit
									</a>
									<button data-id="<?php echo $m['idrombel'];?>" class="btn btn-xs btn-danger btn-flat btnHapus">
										<i class="fas fa-trash-alt"></i>&nbsp;Hapus
									</button>
								</td>
							</tr>
